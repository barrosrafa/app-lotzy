...
describe('GameGenerator',()=>{it.each([15,16,17,18,19,20])('gera %i dezenas válidas',k=>{const g=new GameGenerator(new Sequence()).draw(k,Array.from({length:25},(_,i)=>i+1));expect(g).toHaveLength(k);expect(new Set(g).size).toBe(k);expect(g).toEqual([...g].sort((a,b)=>a-b));});it('gera amostra ponderada sem duplicatas',()=>{const g=new GameGenerator(new Sequence()).weightedDraw(15,Array.from({length:25},(_,i)=>i+1),new Map([[1,100],[2,1]]));expect(g).toHaveLength(15);expect(new Set(g).size).toBe(15);});});

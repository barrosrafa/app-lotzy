...
export class GameAnalyzer {
  ...
  analyze(game: Game): GameMetrics {
    ...
    const primes = sorted.filter((n) => PRIMES.has(n));
    const fib = sorted.filter((n) => FIBONACCI.has(n));
    return {
      size: sorted.length,
      sum: sorted.reduce((a, b) => a + b, 0),
      evens: sorted.filter((n) => n % 2 === 0).length,
      odds: sorted.filter((n) => n % 2 !== 0).length,
      primes: primes.length,
      primeNumbers: primes,
      fibonacci: fib.length,
      fibonacciNumbers: fib,
      frame: sorted.filter((n) => FRAME.has(n)).length,
      core: sorted.filter((n) => CORE.has(n)).length,
      maxConsecutiveRun: maxRun,
      popularity: this.scorer.score(sorted),
    };
  }
}
